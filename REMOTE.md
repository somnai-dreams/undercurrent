...
Accepting an invitation exposes no conversations. Each owner shares exact native conversation IDs per contact. The same grants govern discovery, incoming messages, and which local conversations can send to that contact. A sender's machine is authenticated through its contact credential; its conversation identity is that machine's assertion. A remote peer supplies no user approval or additional authority.
...
The sender never treats relay acceptance as native submission. A successful native handoff still does not mean an agent read or admitted the message. An HTTP transport failure can also be uncertain when forwarding cannot be ruled out. There is no automatic message retry, deduplication guarantee, ordering guarantee, or offline delivery.

The relay stores machine credentials, contact relationships, and unused invitations in one private JSON file. Keep that file across restarts. Run one relay process per state file. Connections and pending receipts exist only in memory. Each machine stores one remote identity and individual sharing grants alongside its existing local registrations. There are no remote copies of native transcripts.
