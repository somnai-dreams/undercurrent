...
Idle conversations remain registered and can still receive direct messages. Installed prompt, tool-completion and stop hooks refresh existing registrations; they do not undo `uc leave`. Session-end hooks remove registrations; automatic startup/resume can rejoin. Crashed or quiet conversations disappear from default discovery after 30 minutes. With missing or unreviewed hooks, active conversations can also age out; rejoin to refresh. Do not add polling or infer liveness from a socket file.
