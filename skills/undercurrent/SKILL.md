...
`uc config` shows the current directory's effective project policy. `join: auto` registers through installed startup hooks; `manual` requires `uc join --name <label>`; `off` disables participation. Project fields override global defaults, and a project allow-list replaces the global list. `allow: ["self"]` allows only conversations in the same project; an empty list permits nothing. Joining announces the conversation to the local machine and already paired contacts. An allow-list gates messages, not discovery.
