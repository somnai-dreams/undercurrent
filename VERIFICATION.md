...
`bun run check` passes strict TypeScript 7, type-aware lint, and 69 tests (576 assertions), including the remote prototype and project-policy checks below. Local coverage includes exact native addresses, duplicate labels, simultaneous registrations, malformed data, attachment and rejoining, literal multiline and Unicode text, CLI input sources, native failures, and timeouts without retries. Socket and loopback relay fixtures need the host's normal permission to open their local listeners.
...
This checkout now has a private `.undercurrent.json` (`join: auto`, allowing only this project's canonical root), project-local Codex and Claude hooks, and their updated skills. `uc config` confirms the effective policy, and `uc peers` lists the two existing dogfood registrations as peers with unchanged native addresses and Claude socket. No global participation or external access has been enabled. Global policy supplies defaults; automatic startup still requires installation of the native integration in each participating project. Generated local integration files are ignored; the reusable installer and skill are tracked.
